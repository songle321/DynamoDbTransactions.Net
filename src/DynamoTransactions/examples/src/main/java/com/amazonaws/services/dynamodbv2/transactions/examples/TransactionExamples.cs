...
//ORIGINAL LINE: public void setup() throws Exception
		...
		public virtual void conflictingTransactions()
		{
			...
			catch (TransactionRolledBackException)
			{
				...
				t1.deleteAsync(); // Delete it, no longer needed
			}
			...
			GetItemResponse result = txManager.getItemAsync((new GetItemRequest()).withTableName(EXAMPLE_TABLE_NAME).withKey(item1Key), Transaction.IsolationLevel.UNCOMMITTED);
			print("Notice that t2's write to Item1 won: " + result.Item);

			result = txManager.getItemAsync((new GetItemRequest()).withTableName(EXAMPLE_TABLE_NAME).withKey(item3T2), Transaction.IsolationLevel.UNCOMMITTED);
			print("Notice that t2's write to Item3 also went through: " + result.Item);

			result = txManager.getItemAsync((new GetItemRequest()).withTableName(EXAMPLE_TABLE_NAME).withKey(item2T1), Transaction.IsolationLevel.UNCOMMITTED);
...
//ORIGINAL LINE: public void badRequest() throws RuntimeException
		...
		public virtual void readThenWrite()
		{
			...
			IDictionary<string, AttributeValue> item1 = t1.getItemAsync(new GetItemRequest()
...
//ORIGINAL LINE: @DynamoDBVersionAttribute public Nullable<long> getVersion()
		...
		public virtual void readCommittedWithMapper()
		{
			...
			ExampleItem keyItem = new ExampleItem();
			keyItem.ItemId = "Item1";
			ExampleItem item = txManager.loadAsync(keyItem, Transaction.IsolationLevel.COMMITTED);
